...
export default function App() {
  ...
  const [pupils, setPupils] = useState<Pupil[]>(() => {
    ...
    return saved ? JSON.parse(saved) : INITIAL_PUPILS;
  });
  ...
  const [expenditures, setExpenditures] = useState<Expenditure[]>(() => {
    const saved = localStorage.getItem('kva_expenditures') || localStorage.getItem('sanyu_expenditures');
    return saved ? JSON.parse(saved) : INITIAL_EXPENDITURES;
  });

  const [miscIncomes, setMiscIncomes] = useState<MiscellaneousIncome[]>(() => {
    const saved = localStorage.getItem('kva_misc_incomes') || localStorage.getItem('sanyu_misc_incomes');
    return saved ? JSON.parse(saved) : INITIAL_MISC_INCOME;
  });
  ...
  const [activeTab, setActiveTab] = useState<
    ...
    'timetables' | 'shuttle' | 'assets' | 'simulator' | 'predictions' | 'role-matrix' | 'database' | 'curriculum' | 'idcards'
  >('dashboard');
  ...
  const [newStaffClass, setNewStaffClass] = useState<NurseryClass | 'All'>('Baby Class');
  ...
  const handlePromoteYear = () => {
    ...
    if (promoYearClassLevels) {
      const nextClassMap: Record<NurseryClass, NurseryClass> = {
        'Baby Class': 'Middle Class',
        'Middle Class': 'Middle Class', // Wait, let's promote Middle -> Top. Map should be: Baby -> Middle, Middle -> Top, Top -> Primary One, etc. Let's fix this list!
      ...
      // Wait, let's fix the Middle Class mapping above: Middle Class maps to Top Class! Let's double check. Yes.
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F5F5F4] text-stone-900 antialiased font-sans">
      ...
      <aside className="w-full md:w-64 bg-[#0F172A] border-b md:border-b-0 md:border-r border-slate-800 flex flex-col justify-between shrink-0 text-slate-300 font-sans">
        <div>
          ...
          <nav className="px-3 space-y-1">
            ...
            >
              <Smartphone className={`w-4 h-4 shrink-0 transition-colors ${activeTab === 'simulator' ? 'text-sky-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
              Mobile SMS Simulator
            </button>
        ...
        <div className="px-4 pb-0 pt-2">
          <div className="bg-slate-800/65 border border-slate-700/40 p-3.5 rounded-2xl text-[11px] text-slate-300 space-y-2 font-normal">
            ...
            <div className="space-y-1 font-semibold text-[10px] font-mono">
              ...
              <div className="flex justify-between items-center text-slate-300">
                <span>WhatsApp:</span>
                <span className="text-emerald-400 select-all font-bold">0703209254</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-slate-800/50 border border-slate-800/80 p-4 rounded-2xl text-[11px] text-slate-300 space-y-2.5 font-semibold">
            ...
            <div className="pt-2 border-t border-slate-800 space-y-1.5">
              ...
              <button
                ...
                className="w-full bg-slate-800 hover:bg-slate-750 text-slate-200 border border-slate-700 py-1.5 px-2.5 rounded-xl font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer"
              >
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#F5F5F4]">
        ...
        <header className="h-20 bg-white border-b border-stone-200/80 px-6 flex items-center justify-between shadow-xs">
          <div>
            ...
            <h2 className="text-lg font-black tracking-tight text-stone-900">
              ...
              {activeTab === 'simulator' && "Simulated Parent/Teacher Multi-Persona App"}
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Mini avatar */}
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-stone-800 to-stone-950 flex items-center justify-center font-black text-xs text-white border border-stone-700 shadow-sm">
              FN
            </div>
          </div>
        </header>
        ...
        <div className="p-6 space-y-8 flex-1 overflow-y-auto max-w-7xl w-full mx-auto">
                  ...
                  <div className="p-6 max-h-[380px] overflow-y-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-[#F2EDE4] text-sm text-[#3D2B1F]">
                        {([
                          { id: 'Baby Class', label: 'Baby Class (3-4 Yrs)' },
                          { id: 'Middle Class', label: 'Middle Class (4-5 Yrs)' },
                          { id: 'Top Class', label: 'Top Class (5-6 Yrs)' },
                ...
                <div className="lg:col-span-4 bg-[#F2EDE4] rounded-3xl p-6 flex flex-col gap-4">
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-[#E0D8CC] shadow-xs">
                    ...
                    <p className="text-[11px] text-[#7D6B5D] leading-relaxed">
                      Please check student requirement lists. Some Baby Class students are yet to provide Maize Flour for porridge porridge meals.
                    </p>
                  </div>
                  ...
                  <div className="mt-auto bg-[#1E293B] p-4.5 rounded-2xl text-white space-y-2.5 border border-slate-700/50">
                    ...
                    <div className="space-y-1.5 text-xs font-semibold">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <a href="https://wa.me/256703209254" target="_blank" rel="noreferrer" className="font-mono text-emerald-400 text-[11.5px] hover:text-emerald-300 transition-colors bg-slate-900/60 px-2 py-0.5 rounded border border-slate-850 font-bold">0703209254</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#E0D8CC]">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#7D6B5D]">Upload your school logo to print directly on learner report cards</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="md:col-span-9 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5 ml-auto text-[10px] text-[#7D6B5D] font-bold">
                        ...
                        <button 
                          type="button"
                          onClick={() => setSchoolLogo('https://images.unsplash.com/photo-1546410531-bb4caa6b424d?w=120&auto=format&fit=crop&q=60')}
                          className="px-2 py-1 rounded bg-[#F2EDE4] hover:bg-[#E0D8CC] transition-all text-[#5A3E2B]"
                        >
                          Schooled Kids
                        </button>
                        <button 
                          type="button"
                          onClick={() => setSchoolLogo('https://images.unsplash.com/photo-1594608661623-aa0bd3a69d28?w=120&auto=format&fit=crop&q=60')}
                          className="px-2 py-1 rounded bg-[#F2EDE4] hover:bg-[#E0D8CC] transition-all text-[#5A3E2B]"
                        >
                          Academy Books
                        </button>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        <option value="Baby Class">Baby Class Only</option>
                        <option value="Middle Class">Middle Class Only</option>
                        <option value="Top Class">Top Class Only</option>
                      ...
                      <div className="space-y-3 mt-4 pt-3 border-t border-[#E0D8CC]/50">
                        ...
                        <div className="space-y-1 bg-white">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {(['Baby Class', 'Middle Class', 'Top Class', 'Primary One', 'Primary Two', 'Primary Three', 'Primary Four', 'Primary Five', 'Primary Six', 'Primary Seven', 'All'] as const).map((lvl) => (
                              <button
                                key={lvl}
                        ...
                        <div className="flex justify-between items-center pt-2 gap-2 border-t border-[#E0D8CC]/40">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setNewStaffClass(member.assignedClass || 'Baby Class');
          ...
          {/* TAB 9: SIMULATOR */}
          {activeTab === 'simulator' && (
            <MobileSimulator studentsCount={totalStudentsCount} presentCount={presentStudentIds.length} />
          )}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[11px] text-[#7D6B5D] leading-normal">
                          Trigger end-of-year class promotions! All pupils in Baby Class progress to Middle Class, Top Class advances to Primary One, and student ages increase automatically.
                        </p>
                      </div>
                    </div>
